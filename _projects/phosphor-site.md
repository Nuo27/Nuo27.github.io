...
- Made every page **front-matter-driven**: content lives in YAML, and layouts compose sections through `{% if page.X %}` guards - omit a key and that section simply doesn't render, so one layout serves many page shapes.
...
- **One layout, many pages.** Driving sections from front matter via `{% if page.X %}` guards means the same layout file powers a full about page, a sparse contact page, and everything in between - omit a YAML key and the section vanishes, with no empty markup left behind.
